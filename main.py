...
# 🛠️ تم إضافة الـ ID الخاص بك بنجاح هنا لفتح لوحة التحكم لك وحدك
ADMIN_ID = 8460989245  
...
def start_command(message):
    ...
    # حفظ المستخدم في القائمة تلقائياً عند الضغط على start
    save_user(chat_id)
...
# استقبال نص الإذاعة من الأدمن أو استقبال روابط التحميل من المستخدمين
@bot.message_handler(func=lambda msg: True)
def handle_all_messages(message):
    chat_id = message.chat.id
    
    # 1. إذا كان المطور يقوم بالإذاعة
    if chat_id == ADMIN_ID and admin_state.get(chat_id) == "waiting_broadcast":
    ...
    # 2. إذا كان مستخدم عادي يرسل رابطاً للتحميل
    if user_platform.get(chat_id) is not None:
